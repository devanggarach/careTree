...
<body>
 ...
 <?php include_once"ngo_header_trans_sign.php";?>
        ...
        <div class="bottom-text">
          Don't have NGOs account? <a href="n_register.php"><b>NGOs REGISTER </b></a>
        </div>
...
if (isset($_POST['l_login'])) {
        ...
        $flage=1;
        // echo "$l_email";
        $sql= "SELECT l_id,l_name,l_email,l_password,l_mobile,l_land, l_pincode,l_city,l_state,datetimee FROM n_login WHERE 1;";
        // echo "$l_email";
        // echo "$l_password"; 
        ...
        while ($row = mysqli_fetch_assoc($result)){
            if ($row['l_email']==$l_email){
                if ($row['l_password']==$l_password) {
                   ...
                   $_SESSION["n_id"] =$row['l_id'];
                   $_SESSION['n_mail']=$row['l_email'];
                    echo "<script> alert('login Success...');</script>";
                    echo "<script type='text/javascript' language='Javascript'>window.open('ngoindex.php', '_self');</script>";
