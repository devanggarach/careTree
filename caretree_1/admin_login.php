...
<?php include_once"header_trans_sign.php";?>
        ...
        <div class="bottom-t">
          <a href="forgotpassord.php">Forgot password?</a>
        </div>
        
        <input type="submit" name="a_login" class="logbtn" value="LOGIN">

        <div class="bottom-text">
         Login as Citizen <a href="login.php"><b>CITIZEN LOGIN </b></a>
        </div>
        <div class="bottom-text">
...
if (isset($_POST['a_login'])) {
        ...
        $l_email = $_POST['a_email'];
        $l_password =$_POST['a_password'];
        $flage=1;
        
        $sql= "SELECT `a_id`, `a_name`, `a_pass` FROM `a_login` WHERE 1";
        // echo "$l_email";
        // echo "$l_password"; 
        $result=mysqli_query($conn,$sql);
        while ($row = mysqli_fetch_assoc($result)){
            if ($row['a_name']==$l_email){
                if ($row['a_pass']==$l_password) {
                    $flage=0;
                   ...
                   $_SESSION["a_id"] =$row['a_id'];
                    ...
                    echo "<script type='text/javascript' language='Javascript'>window.open('adminindex.php', '_self');</script>";
